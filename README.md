...
- Coverage: `pnpm test:coverage` (Vitest + V8 outputs under each workspace `coverage/` directory). After it finishes, refresh the markdown summary with `pnpm coverage:report -- --write` so `reports/coverage-baseline.md` matches the latest coverage run; CI will also run `pnpm coverage:report` to enforce the values in `reports/coverage-threshold.json` (bump those numbers when you intentionally raise the floor).
