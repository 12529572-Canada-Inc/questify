...
- Fixed TypeScript lint errors in OAuth handlers (facebook.get.ts, google.get.ts, oauth.ts) by using the correct runtime path imports (`nuxt-auth-utils/runtime/server/lib/oauth/*`) with `@ts-expect-error` directives, resolving both type checking and module resolution warnings.
